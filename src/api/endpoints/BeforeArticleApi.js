...
const BeforeArticleApi = entryApi.injectEndpoints({
  endpoints: (build) => ({
    ...
    confirmApplication: build.mutation({
      query: (token) => ({ url: `/before-article/confirm/${token}`, method: "get" }),
    ...
    cancelApplication: build.mutation({
      query: (token) => ({ url: `/before-article/cancel/${token}`, method: "get" }),
      invalidatesTags: ["beforeArticle"],
      transformResponse: (response) => {
        return response.data;
      }
    ...
    downloadApplication: build.query({
      query: (token) => ({ url: `/before-article/download/${token}`, method: "get" }),
      transformResponse: (response) => {
        return response.data;
      }
    })
