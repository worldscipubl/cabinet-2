...
const ArticlePayment = ({ articleId = "" }) => {
  ...
  const handleByLink = async (method) => {
    ...
    console.log(data);
    await paymentByLink(data).unwrap()
      .then((response) => {
  ...
  const handleByPdf = async (method) => {
    if (!method) return;
    const data = { articleId, paymentMethod: method };
    await paymentByPdf(data).unwrap()
      .then((response) => {
        if (!response?.data) return;
        const { url } = response.data;
        window.open(url, "_blank").focus();
  ...
  const ArticlePaymentActions = ({ status = "", method }) => {
    ...
    return <div className="article-payment-card__col">
      <button className="article-payment-card__row button button_type_main active"
      ...
      <button className="article-payment-card__row button button_type_main"
  ...
  return (
    <div>
      {data.map(({ amount, currency, name, paymentMethod, paymentStatus }) => (
        <Card className="article-payment__item" appearance={{ type: "paper" }} key={name + paymentStatus}>
          <div className="article-payment-card">
            <div className="article-payment-card__col">
              <h3 className="text text_size_subtitle text_weight_bold">
                {name} <br />
                <span className="text_color_red text_size_accent">{paymentStatuses[paymentStatus]}</span>
              </h3>
              <p className="article-payment-card__price text text_size_extra-title text_weight_bold">
                {amount} <span>{currency}</span>
              </p>
            </div>
            <ArticlePaymentActions status={paymentStatus} method={paymentMethod} />
          </div>
        </Card>))}
    </div>
  );
