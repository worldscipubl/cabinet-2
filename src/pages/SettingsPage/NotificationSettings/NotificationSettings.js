...
import {useGetTriggersQuery, useToggleTriggerMutation} from "../../../api/endpoints/NotificationsApi";
...
const NotificationSettings = () => {
    ...
    if (isLoading) return (
        [1, 2, 3, 4, 5].map((key) => (
            <FormFieldRow className="brief-form__title" key={key}
                          label="Уведомления" isLoading={isLoading}>
                <Toggle/>
            </FormFieldRow>
        ))
    )
    ...
    return (
        data?.map(({categoryId, name, items}) => (
...
const NotificationSettingsWrapper = (Component) => ({...props}) => (
