...
const DialogCashOut = ({className}) => {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        setLoading(true);
        const data = {walletId: values?.['walletId'], sum: Number(values?.['sum'])};
        cashOutMoney(data).unwrap()
            .catch((err) => {
                console.log(err);
            })
            .finally(() => {
    ...
    return (
        <Fragment>
            ...
            <Dialog className={classNames(cn.Wrapper)} open={isOpen} setOpen={setOpen}>
                <DialogHeader label="Вывод денежных средств" handleClose={() => setOpen(false)}/>
                <DialogBody isLoading={isLoading}>
                    <FieldBuilder
                        className={classNames(cn.FieldItem)}
                        name="walletId"
                        // isLoading={isLoadingWallets}
                        type="select" label="Выберите карту"
                ...
                <DialogSubmit handleSubmit={handleSubmit} error={error}
                              disable={isLoading} label="Вывести"/>
            </Dialog>
        </Fragment>
    );
};
...
const WalletItem = ({item, onClick}) => {
    ...
    return (
        <li className={classNames(cn.option, "text text_size_default")}
