...
const SelectOption = ({ className, handleSelect, handleOption, title, options }) => {
  ...
  return (
    <div className={classNames(cn.Wrapper)}>
      <div className={classNames(cn.Header)}>
        <IonIcon className={classNames(cn.BackBtn)} name="arrow-back-outline" onClick={() => handleSelect("")} />
        <span className={classNames(cn.Title, "text")}>{title}</span>
      </div>
      <ul className={classNames(cn.List)}>
        {options && options.map(({ id, label, icon }) => (
          <li className={classNames(cn.Item)} key={id} onClick={() => handlerOption(id)}>
