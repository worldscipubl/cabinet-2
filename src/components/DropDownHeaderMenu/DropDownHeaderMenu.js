...
const DropDownHeaderMenu = ({
  ...
  useEffect(() => {
    ...
    const offsetRight =
      window.innerWidth - (anchor.offsetLeft + anchor.offsetWidth) || 0;
    currentEl.style.setProperty("--offset-right", `${offsetRight}px`);
  }, [anchorEl]);

  return (
    <Modal
      className={classNames(cn.Inner, className)}
      classNameWrapper={classNames(cn.Wrapper)}
      open={open}
      setOpen={setOpen}
      ref={ref}
    >
      <div className={classNames(cn.Content)}>{children}</div>
    </Modal>
  );
};
