...
} from "../../api/endpoints/BeforeArticleApi";
...
const UploadCard = ({ article }) => {
  ...
  const { subject, token, file, dateCreate } = article || {};
    ...
    <div className={classNames(cn.Wrapper, { [cn.isLoading]: isLoading })}>
      <div className={classNames(cn.Container)}>
        ...
        <div className={classNames(cn.BtnGroup)}>
          <button className="button button_type_table" onClick={handleConfirm}>
            Подтвердить
          </button>
          <button className="button button_type_table" onClick={handleClose}>
            Закрыть
          </button>
        </div>
      </div>
