...
const FormField = ({
  ...
  const submitField = () => {
    ...
    handleFieldSubmit(name, value).catch((err) => {
      resetField();
      setError(err);
    });
  };
  ...
  return (
    ...
    >
      <>
        {isReadOnly && !!error && (
          <img
            className={classNames(styles.Img, styles.startIcon)}
            src={errorImg}
            alt="start-icon"
          />
        )}
        ...
        {isReadOnly && !isViewOnly && (!defaultValue || fileName) && (
          <i
            className={classNames(
              "button button_type_tabs active",
              styles.actionBtn
            )}
            onClick={() => {
              if (fileName) {
              ...
              if (propsInput.type === "file") inputRef.current.click();
              else {
                if (inputRef) inputRef.current.focus();
                setReadOnly(false);
              }
